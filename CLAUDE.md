...
6. **본답 push + queue done + 검증** — 본답은 `━━━━━━━━━━━━━━━` 구분선 시작 ([[feedback-helper-discord-newline]] — discord-reply.sh 본답 모드 자동 ZWSP+\n prepend). **자동 reply** ([[feedback-helper-discord-reply-to]]): bot.py 가 `last-user-msg-id.txt` 캐시 + `discord-reply.sh` bare body 모드가 자동 `message_reference` payload 빌드 (#946). 명시적 disable: `--no-reply`. **writing marker** (#1095): 본답 push 직전 `bash /home/mobruji/.mobruji/discord-reply.sh --writing-marker $(cat ~/.mobruji/helper-current-target.txt)` 으로 ✍️ reaction + typing indicator 활성. 본답 push 직후 `bash /home/mobruji/.mobruji/discord-reply.sh --writing-done $(cat ~/.mobruji/helper-current-target.txt)` 으로 ✍️ reaction 해제 — 사용자에게 "답 작성 시작/완료" 가시화. 자동 hook 옵트인은 `BOT_WRITING_AUTO_HOOK_ENABLED=1` env 로 명시 호출 없이도 강제 (default off, 명시 호출 룰 학습 안정화 후 운영자 전환). push 후 message_id 행 `status: done` 갱신 + `grep '"status": "pending"' ~/.mobruji/helper-queue.jsonl` 으로 0건 확인. thread 생성했다면 마지막 `--auto-thread "[done]"` push.
