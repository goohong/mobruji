...
0. **turn-start wrapper 호출** (#1014, #1128 writing marker 확장) — `bash /home/mobruji/.mobruji/helper-turn-start.sh`. **매 turn 첫 명령 의무**. 6 액션 자동 수행 (target freeze + **✍️ writing marker ON** + cycle-status 요약 + user-presence 표시 + queue pending 표시 + 다음 액션 reminder). 학습 의존 → wrapper 강제. graceful (exit 1 안 함 — helper turn 안 깨짐). 아래 1-6 단계는 wrapper 가 처리한 1-2단계 (queue append/target freeze) + 추가된 ✍️ ON 호출 (#1128) 을 명시적으로 재확인.
...
6. **본답 push + queue done + 검증** — 본답은 `━━━━━━━━━━━━━━━` 구분선 시작 ([[feedback-helper-discord-newline]] — discord-reply.sh 본답 모드 자동 ZWSP+\n prepend). **자동 reply** ([[feedback-helper-discord-reply-to]]): bot.py 가 `last-user-msg-id.txt` 캐시 + `discord-reply.sh` bare body 모드가 자동 `message_reference` payload 빌드 (#946). 명시적 disable: `--no-reply`. **writing marker** (#1095 도입 + #1128 timing 정정): ✍️ ON 은 step 0 wrapper 가 turn 시작 시점에 자동 호출 ([[feedback-helper-writing-marker-timing]], 사용자 16:21-24 directive — turn 내내 ✍️ 가시화). wrapper 미사용 시 폴백으로 본답 push 직전 `bash /home/mobruji/.mobruji/discord-reply.sh --writing-marker $(cat ~/.mobruji/helper-current-target.txt)` 호출. 본답 push 직후 `bash /home/mobruji/.mobruji/discord-reply.sh --writing-done $(cat ~/.mobruji/helper-current-target.txt)` 으로 ✍️ reaction 해제 — 사용자에게 "답 작성 시작/완료" 가시화. 자동 hook 옵트인은 `BOT_WRITING_AUTO_HOOK_ENABLED=1` env 로 명시 호출 없이도 강제 (default off, 명시 호출 룰 학습 안정화 후 운영자 전환). push 후 message_id 행 `status: done` 갱신 + `grep '"status": "pending"' ~/.mobruji/helper-queue.jsonl` 으로 0건 확인. thread 생성했다면 마지막 `--auto-thread "[done]"` push.
