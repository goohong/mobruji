...
/**
 * 시드 JSON(`classpath:/songs-seed.json`)을 부팅 시 한 번 적재. 이미 데이터가 있으면 skip (idempotent).
 * 테스트(test 프로파일)에서는 자동 적재하지 않는다 — 각 테스트가 자체 데이터를 셋업.
 */
@Slf4j
@Component
@Profile("!test")
...
public class SongSeedLoader implements ApplicationRunner {
    ...
    public void run(final ApplicationArguments args) throws IOException {
        if (songRepository.count() > 0) {
            log.info("Song table already populated ({} rows), skipping seed", songRepository.count());
            return;
        }
