/**
 * /history 페이지 query/source 전환 race 가드 — fixture refixture 재진입 (closes #1093).
 *
 * 본 파일 도입 사유 (PR #1069 closed → fixture 통합 후 재진입):
 *   - PR #1069 (closed) — `/history` 페이지 컨텍스트 race guard 5건 신설안.
 *     base 가 PR #1061 stack 이었고 inline fixture builder (`buildLocalEntry` /
 *     `buildBackendEntry` / `buildVoiceRangeSnapshot`) 가 `page.test.tsx` 와 중복.
 *   - PR #1090 (refactor) — `web/lib/test-fixtures/history.ts` fixture builder 추출.
 *     `page.test.tsx` 가 fixture 모듈로 통합 완료.
 *   - 본 파일 — PR #1090 fixture + PR #1057/#1061 race-helpers 둘 다 reuse 한 형태로
 *     PR #1069 race guard 5건을 재진입한다. inline builder 0건.
 *
 ...
 *     는 의도된 더미값.
 *   - voiceRangeLowMidi/highMidi 는 음역대 PII 가능성 — fixture default 52/70 은 더미.
 */
...
import type { RecommendationHistoryListResponse } from "@/lib/api/recommendationHistory";
import type { VoiceRangeHistoryResponse } from "@/lib/api/voiceRangeHistory";
...
describe("/history 페이지 query/source 전환 race 가드 (fixture refixture, #1093)", () => {
  ...
  it("시나리오 1: sessionId null 상태에선 BE 호출 0건 + localStorage fallback 카드 렌더", async () => {
    const tenMinutesAgo = minutesAgoIso(10);
    historyMock.set({
      recommendations: [
        buildLocalEntryFromSongIds("e-1", tenMinutesAgo, [10, 20, 30]),
      ],
    });
    ...
    // localStorage fallback 카드 — fixture default 곡명 "곡-10" 노출.
    expect(await screen.findByText("곡-10")).toBeInTheDocument();
  ...
  it("시나리오 2: BE 응답 도착 시 localStorage → BE source 전환 (스펙 §5-6 우선순위)", async () => {
    ...
    historyMock.set({
      recommendations: [
        buildLocalEntryFromSongIds("e-1", tenMinutesAgo, [10]),
      ],
    });
    ...
    // 응답 도착 전 — localStorage fallback 카드 노출 (fixture default 곡명 "곡-10").
    expect(screen.getByText("곡-10")).toBeInTheDocument();
    expect(screen.queryByText("be-곡-100")).not.toBeInTheDocument();

    // BE 응답 도착 — source 전환. fixture buildBackendEntry 가 buildRecommendedSong
    // 사용 — default 곡명은 "곡-{id}". BE 카드 식별을 위해 명시적으로 다른 곡명을 준다.
    await act(async () => {
  ...
  it("시나리오 4: pending 중 페이지 unmount → React state update warning 0건", async () => {
    ...
    await act(async () => {
      ...
      vrDeferred.resolve(buildVoiceRangeListResponse([]));
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it("시나리오 5: voice-range BE snapshots 2건 fresh → BE summary 사용 (스펙 §3 source-of-truth)", async () => {
    const tenMinutesAgo = minutesAgoIso(10);
    const oneHourAgo = minutesAgoIso(60);
    sessionMock.set({ sessionId: "test-session-id" });
    historyMock.set({
...
/**
 * 본 파일에서 사용된 race-helpers (PR #1057/#1061) + fixture (PR #1090) reuse 카탈로그:
 *   - race-helpers:
 ...
 *   - fixture (PR #1090):
 *     * buildLocalEntryFromSongIds — 시나리오 1/2/3/4/5 = 5회 (모든 시나리오)
 *     * buildBackendEntry — 시나리오 2/4 = 2회
 *     * buildBackendListResponse — 시나리오 2/4/5 = 3회
 *     * buildVoiceRangeSnapshot — 시나리오 5 = 2회 (snapshots 2건)
 *     * buildVoiceRangeListResponse — 시나리오 2/4/5 = 3회
 *     * minutesAgoIso — 시나리오 1/2/3/4/5 = 5회 (모든 시나리오)
 *
 * PR #1069 (closed) 대비 LOC 절감:
 *   - inline `buildLocalEntry` (35 LOC) / `buildBackendEntry` (35 LOC) /
 *     `buildBackendListResponse` (5 LOC) / `buildVoiceRangeSnapshot` (12 LOC) /
 *     `buildVoiceRangeHistoryResponse` (5 LOC) / `ridFromSeed` (3 LOC) =
 *     약 95 LOC 가 fixture 모듈로 옮겨가 본 파일에서 제거됨.
 *   - 추가 부수 효과: 도메인 필드 추가 시 fixture 한 곳만 갱신 (page.test.tsx 와 sync).
 */
