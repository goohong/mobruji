...
/**
 * 추천 의도 모드 토글 (persona-expansion-social-emotional.md §5-6, 이슈 #1600 / #1844).
 *
 * 기존 추천 화면(음역대 + 분위기/나이대)에 "감정 축" 의도 모드 진입을 더한다 — "안 망할 곡"
 * (P-E 안전곡)과 그 반대축 "고음 질러 박수받기"(P-F 과시·킬링파트). spec §8 Q2(온보딩
 * `PersonaEntryPath` 통합 vs 추천 화면 내 모드 선택)는 미결정 상태라, 이슈 요구사항("기존 추천
 * 화면에 의도 모드 선택 진입")에 맞춰 **추천 화면 내 토글**(Q2 선택지 b)을 1차 가설로 둔다.
 *
 * 동작:
 *  - 두 모드는 **상호 배타**다. 한 모드를 켜면 다른 모드는 자동으로 꺼진다(persona 단일 값).
 ...
 *  - P-D 시퀀스는 별도 화면(RecommendModeGroup 의 이동형 진입), P-G 듀엣은 후속 이슈.
 */
