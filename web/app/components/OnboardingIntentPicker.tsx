 ...
 *
 * graceful fallback (PR 2 단독):
 *   - F1/F2/F3 자식 경로(가이드 측정 / 연습 모드 / 분위기 모드)가 아직 미구현이라
 *     세 카드 모두 기존 측정 wizard(`/voice-range/auto`)로 연결한다. 경로별 목적지는
 *     `PATH_DESTINATION` 한 곳에 모아 두어, 자식 spec 진척 시 한 줄로 갈아끼운다
 *     (PR 3/4 wiring).
 ...
 * 경로별 라우팅 목적지.
 *
 * 현재는 자식 경로 미구현이라 전부 기존 측정 wizard 로 fallback 한다. F1/F2/F3
 * 화면이 생기면 이 매핑만 갈아끼운다(예: BEGINNER → `/voice-range/auto?tour=1`).
 */
const PATH_DESTINATION: Record<PersonaEntryPath, string> = {
  BEGINNER: "/voice-range/auto",
  PRACTICE: "/voice-range/auto",
  MOOD: "/voice-range/auto",
};
