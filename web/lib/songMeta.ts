...
import type { Mood } from "@/lib/api/recommendation";
...
/**
 * Mood enum → 한국어 라벨 (#1764).
 *
 * BE `com.mobruji.song.domain.Mood` 6종(SCREAMING_SNAKE)을 곡 상세/검색에서 그대로
 * 노출하면 코드값 노이즈가 된다. RecommendFilters 의 분위기 선택 라벨과 같은 표현을 쓴다.
 */
...
/**
 * 분위기 코드값을 한국어 라벨로 변환한다. 빈 값이거나 매핑에 없는 코드는 `null` 을 돌려준다 —
 * 호출 측은 `null` 일 때 분위기 칩 자체를 그리지 않아 내부 코드 원문 노출을 막는다.
 */
