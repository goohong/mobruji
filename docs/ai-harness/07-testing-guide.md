...
### 7-1.3 마이그레이션(Flyway 도입 완료)

Flyway 도입 완료 (ADR-0009, `backend/src/main/resources/db/migration/` V1~V7). 신규 DDL 변경은 새 `V<N>__<요약>.sql` 파일로 추가한다.

**Flyway 기준 룰**
- 머지된 V 마이그레이션은 **수정 금지**. 잘못됐으면 새 V<N+1>__... 로 보정 마이그레이션을 추가한다 (Flyway checksum 보호).
- DDL 변경(컬럼 추가/제거/타입 변경)은 **rollback 가능**해야 한다. PR 본문에 rollback 절차(역방향 SQL 또는 후속 보정 V 번호 계획)를 적는다.
- **Zero-downtime 검증**: (1) 기존 코드가 새 컬럼 없이도 동작 (2) 새 코드가 기존 컬럼만 있는 row를 처리 — 두 케이스 모두 통합 테스트 1건씩.
- 신규 V 마이그레이션 PR 본문에는 적용되는 V 번호 + ADR-0009 백링크를 명시한다.
