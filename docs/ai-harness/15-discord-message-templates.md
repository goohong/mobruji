...
> Discord 채널 #모부르지 운영 메시지 표준 + 사용자 ↔ 본진 양방향 명령 인식 규약.
> 관련 문서:
...
- **메시지 출처 3종**:
  ...
  3. **본진 직접 (양방향 채팅)** — 사용자와의 인터랙티브 메시지 (§2-3)
- **목적**: 일관된 시각 형식으로 (a) 사용자가 모바일에서 한 눈에 상태 파악, (b) 본진이 fetch 시 명령을 안정적으로 인식.
...
본진이 `plugin_discord` MCP 도구로 직접 send/fetch.
...
본진이 `fetch_messages`로 사용자 메시지를 받을 때 인식하는 패턴.
...
- **평문**: 명령 패턴에 매칭되지 않으면 일반 대화로 처리 (본진이 자연어로 응답).
- **모호한 경우**: 본진이 `❓ 결정 필요` 카드로 confirm 질문 발송.
...
- `/discord:access` 스킬은 사용자가 로컬 터미널에서 직접 실행. 본진이 호출하지 않음.
- `access.json` 직접 수정 금지.

## §5 본진 active 시간대 / 비활성 처리
...
- **본진 평문 reply**: 2000자 (Discord 메시지 한도) 이내.
- **초과 처리**:
  - 자동 분할 (여러 메시지로 나눠 전송)
