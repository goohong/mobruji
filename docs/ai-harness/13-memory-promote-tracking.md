...
| `feedback_discord_polling` | 메모리 유지 | (코드 promote 부적합) | 본진 polling cadence. 사용자 컨텍스트 의존, 24/7 인프라는 plan 20에서 별도 추진 |
...
### 2-4) [P3] 메모리 단축 후보 (본진 영역, race 회피로 본 PR 직접 수정 안 함)
plan 22 이후 추가된 메모리 항목 중 다른 메모리와 중복도가 높아 1줄 단축이 가능한 후보. 본진 사이클에서 처리.

- **`feedback_discord_polling`** — `project_discord_channel`과 한 묶음으로 통합 가능. "본진은 #모부르지 채널만 매 호흡 fetch_messages" 1줄로 압축.
...
처리 가이드: 본진이 다음 사용자 입력 idle 사이클에 메모리 편집 (`~/.claude/projects/.../memory/MEMORY.md` 직접 수정). plan 세션은 본 매트릭스에서만 추적.
