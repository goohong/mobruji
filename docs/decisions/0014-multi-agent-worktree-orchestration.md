...
### 긍정적
- 4 도메인 병렬로 wall-clock 단축. maestro가 직접 코드 작성하던 시점 대비 사이클 throughput 증가.
- 워크트리 분리로 작업 영역(`backend/**` vs `web/**` vs `docs/**`) 충돌 자연스럽게 회피.
- maestro 컨텍스트가 메타(기획/ADR/리뷰 결정)에 집중되어 단일 세션 컨텍스트 폭증 완화.
...
- fe 워크트리는 `npm install` 직후 `node_modules` 심볼릭 링크가 maestro(`mobruji/web/node_modules`) 실체로 다시 swap되어야 한다. 미수행 시 fe sub-agent가 lint/typecheck/test에서 의존성 누락으로 일제 실패 (`feedback-npm-install-symlink-swap`). 의존성 변경 PR은 maestro/fe 어디서 install했든 swap 단계까지 한 사이클로 묶는다.
...
- **(D) 4 sub-agent 외 plan 세션 미도입 (3 워크트리)** — be/fe/rev 3 워크트리 + maestro가 ADR/spec 작성. 메타 작업이 maestro 부담을 폭증시켜 plan 세션 도입으로 분리됨(`project-plan-session-active`, 2026-05-21).
