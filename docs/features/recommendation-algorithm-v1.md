...
| #  | 질문 | 선택지 | 담당/기한 |
|----|------|--------|-----------|
| Q1 | v1 알고리즘 형태 (= 도메인 모델 D3) | (a) 규칙 기반 베이스라인 (본 spec 권장) / (b) Spotify audio-features 임베딩 거리 / (c) LLM 호출 추천 / (d) 하이브리드(규칙 → LLM 재정렬) | @goohong / 2026-05-27 |
| Q2 | 점수 가중치 초기값 | (a) `w1=0.5, w2=0.2, w3=0.15, w4=0.15` / (b) `voiceRangeFit` only(`w1=1.0`) / (c) hand-eval 후 결정 | @goohong / 2026-05-27 |
| Q3 | 콜드스타트(음역대만 입력) 동작 | (a) `w1`만 사용 / (b) 음역 적합곡 중 popularity 상위 노출 / (c) 분위기 미입력 시 "다양 분위기" 강제 다양화 | @goohong / 2026-05-27 |
| Q4 | 다양성 후처리 규칙 | (a) 같은 아티스트 ≤ 2 / (b) 같은 장르 ≤ 4 / (c) (a) + (b) 둘 다 / (d) 적용 안 함 | @goohong / 2026-05-27 |

**추천(작성자 권고, 사용자 검토 대기):**
- Q1 → (a) 규칙 기반 베이스라인. 디버깅·설명 가능성·응답 속도·비용 모두 v1에 우호적. LLM/임베딩은 v2에서.
- Q2 → (a) `0.5/0.2/0.15/0.15`. 음역 적합이 1순위, 분위기·성별·인기는 보조.
- Q3 → (a) `w1`만. 단순함이 디버깅에 유리.
- Q4 → (c) 둘 다 적용. PoC 카탈로그가 작아 한 아티스트가 결과 도배할 위험이 큼.
