...
|---|---|---|---|
| 1 | `user_message_received` | bot.py `on_message` | (a) `~/.mobruji/helper-queue.jsonl` append `status: pending` (b) `~/.mobruji/last-user-msg-id.txt` write (c) 1초 generic auto-ack push (`BOT_AUTO_ACK_TEXT`) (d) emit event `user_message_classified` (5-2 분류기) |
