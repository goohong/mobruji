...
- 2026-06-04: PR C 구현 (#1496) — 시드 30곡 → 100곡 확장(70곡 신규). 장르 균형 발라드30/댄스20/락15/트로트15/팝20, 시대 균형 90s20/00s25/10s30/20s25 default 정확히 충족. 기존 `R&B` 2곡(Beautiful·Some)을 화이트리스트(발라드)로 재분류(§5-5 화이트리스트 정합, 본 §9 권장 반영). lowMidi/highMidi 는 원곡 키·메인 보컬 라인 근거 추정(직접 측정 아님)이라 신규 곡 `metadataConfidence` 0.7~0.8 보수 설정 — 추측 박제 방지. 노래방 곡번호(tjNumber/kyNumber)는 검증 불가분 null(§5-5 "가능 시"). 스키마 게이트(#276) + 적재 통합 테스트 그린. 출처: 본 PR
