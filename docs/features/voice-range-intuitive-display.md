...
## 2) 사용자 시나리오
1. **측정 직후 이해** — 사용자가 음역대를 입력/자동 측정한 뒤 결과 화면에서 "0옥솔 ~ 2옥라 (G2 ~ A4)" 처럼 계이름 병기를 보고, 건반/막대 위에 자기 범위가 칠해진 그림으로 "내가 여기서 여기까지 부르는구나" 를 한눈에 파악한다. (한글 옥타브는 노래방 통념 = SPN − 2, #1856)
...
## 3) 요구사항
### 기능 요구사항
- [ ] 사용자 가시 음역대 표면(측정 결과·확인 화면)에서 **계이름 + 옥타브** 를 SPN 과 병기한다 (예: `2옥도 (C4)` — 한글 옥타브는 노래방 통념 = SPN − 2, #1856). 기존 `web/lib/notes.ts` 헬퍼를 단일 출처로 재사용한다.
...
- 2026-06-05: **한국어 음명 옥타브를 노래방/가창 통념 옥타브로 표기** (#1856).
  - 문제: `midiToKoreanNoteName` 이 SPN 옥타브 숫자를 그대로 한글에 붙여 "미3"·"레5" 로 노출 → 한국 보컬 통념("몇 옥타브 도/레") 카운팅과 어긋나 사용자 혼란(이슈 본문).
  - 결정: 한국 통념 옥타브 = **SPN 옥타브 − 2** (1옥타브 도 = C3, 3옥타브 = C5~B5). 표기는 옥타브를 앞세운 `2옥라`/`3옥도` 형식. SPN(영문 `C4`)은 정확도/학습용으로 별도 유지.
  - 근거: namu.wiki 옥타브/3옥타브 문서. 앵커 — 남자 평균 고음 = 2옥라(A4), 3옥도의 벽 = 3옥도(C5), 여성 기준 고음 = 3옥레(D5).
  - 구현: `notes.ts` 단일 출처(`KOREAN_VOCAL_OCTAVE_OFFSET=2`)만 수정 → 모든 표면(슬라이더·차트·카드·결과) 자동 정합. #1853(picker 하한 G2) 과 동일 밴드 공유.
