...
| #  | 질문 | 선택지 | 담당/기한 |
|----|------|--------|-----------|
| Q1 | 1차 메타데이터 주 출처 (= 도메인 모델 D2) | (a) 수기 시드 JSON only / (b) 수기 시드 + MusicBrainz 보강 / (c) 수기 시드 + Spotify audio-features 보강 / (d) Spotify만 / (e) 크롤링 포함 | @goohong / 2026-05-27 |
| Q2 | 노래방 곡번호(TJ/금영) 입수 방법 | (a) PoC는 비워두고 향후 결정 / (b) 수기 입력 / (c) 노래방 사이트 크롤링(약관 검토 필요) / (d) 사용자 기여 | @goohong / 2026-05-27 |
| Q3 | 곡 음역(`SongRange`) 산정 | (a) 수기 입력 / (b) 원곡 키 + 평균 보컬 음역 휴리스틱으로 추정 / (c) 외부 데이터셋(있으면) / (d) 본 단계 보류, voice-range 매칭은 "키" 만으로 1차 추천 | @goohong / 2026-05-27 |
| Q4 | PoC 시드 데이터 규모/장르 분포 | (a) 50곡 / (b) 100곡 / (c) 200곡+ / 장르: 발라드/댄스/락/힙합/팝/트로트 등 균등 vs 가중 | @goohong / 2026-05-27 |
...
- Q2 → (b) 수기 입력 (시드 100곡 대상에만). 크롤링은 법적 리스크 + 운영 부담으로 후순위.
- Q3 → (d) 본 단계 보류. 1차 추천은 "사용자 음역대가 곡 원곡 키 ±3 semitone에 들어가는가" 정도의 규칙으로 단순화. 본격 `SongRange` 산정은 D3 spec에서.
- Q4 → (b) 100곡, 발라드/댄스/팝 중심 가중. 적은 양으로도 추천 흐름 검증 가능 + 큐레이션 부담 합리적.
