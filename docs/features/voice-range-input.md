...
## 3) 요구사항
### 기능 요구사항
- [x] 사용자가 음역대를 등록할 수 있는 진입 경로를 제공한다 (방식은 §5에서 결정).
- [x] 등록된 음역대를 `VoiceRange`(최저음, 최고음) 형태로 영속화한다.
- [x] 사용자가 자신의 음역대를 조회할 수 있다.
- [x] 사용자가 음역대를 재측정/수동 수정할 수 있다.
- [x] 음역대 단위는 **MIDI note number 또는 과학적 음표 표기법(예: `C4`)** 둘 다 다룰 수 있는 내부 표현을 사용한다.
