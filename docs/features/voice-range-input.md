...
| #  | 질문 | 선택지 | 담당/기한 |
|----|------|--------|-----------|
| Q1 | 음역대 진단 방식 (= 도메인 모델 D1 이관) | (a) 마이크 실측 — 정확도↑/구현 비용↑/마이크 권한 UX 부담 / (b) 자가 진단 곡 — 짧은 가이드 곡을 따라 부르고 사용자가 "어디까지 됐다"를 표시 / (c) 옥타브 분류 선택 — 5~6단계 프리셋 중 고름. 가장 단순, 정확도 낮음 / (d) 하이브리드: 기본 (c), 옵션으로 (a) | @goohong / 2026-05-27 |
| Q2 | 익명 세션 vs 회원가입 (= 도메인 모델 D4 일부) | (a) 익명 세션(쿠키/로컬스토리지)만으로 PoC / (b) 익명 시작 + 선택적 회원가입 / (c) 처음부터 회원 필수 | @goohong / 2026-05-27 |
| Q3 | DB 마이그레이션 도구 | (a) Flyway / (b) Liquibase / (c) PoC 한정 `ddl-auto=update` 후 도입 | @goohong / 2026-05-27 |
| Q4 | 음역대 이력 보관 정책 | (a) 최신 1건만 덮어쓰기 / (b) 변경 이력 전부 보존(분석/UX 회상용) | @goohong / 2026-05-27 |

**추천(작성자 권고, 사용자 검토 대기):**
- Q1 → (d) 하이브리드: 1차 PoC는 (c) 옥타브 분류만 구현해 가장 빨리 추천 흐름 닫고, 차후 (a) 마이크 실측을 옵션으로 추가. 정확도 부족은 §3 비기능 요구사항(30초 내 완료)과 트레이드오프.
- Q2 → (a) 익명 세션. PoC 단계에선 회원가입 없이 흐름을 닫는 것이 학습/검증 측면에서 가장 빠르다.
