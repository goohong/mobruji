...
# rev e2e 단계 2 (dev 배포 E2E 검증) 자동 trigger (#1008,
# spec: docs/features/rev-e2e-2-stages.md §3-2 + docs/features/stage2-dev-deploy-e2e.md).
# 5분 polling — develop 머지된 PR 중 `rev-post-merge-pass` 라벨 없는 항목을
# gh CLI 로 발굴해 nmae tmux pane 에 dev 배포 E2E 검증 launch 알림 inject + Discord push.
# 동일 PR 반복 inject 방지를 위해 in-process debounce (기본 15분).
#
# 명명 (2026-05-30 rev2s-1 / PR #1367 통일 → 2026-06-02 #1461 단계 2 재정의):
#   - 단계 1 = 🟡 Pre-merge review (PR 머지 전)
#   - 단계 2 = 🔵 dev 배포 E2E 검증 (배포본 E2E 점검) — develop dev 배포 후
#     배포본 대상 E2E ← 본 loop 대상. (코드 식별자 rev_post_merge_audit /
#     REV_POST_MERGE_* / 라벨 rev-post-merge-pass 는 유지 — §8 Q3=a / Q5=a)
...
# 중복 방지. 15분이면 단계 2 (dev 배포 E2E 검증) 시작/완료를 기다리기엔 충분.
...
# label 조회 시 사용할 label 이름 — rev sub-agent 가 단계 2 (dev 배포 E2E 검증)
# 통과 시 부여.
...
REV_POST_MERGE_AUDIT_INJECT_TEMPLATE: Final[str] = (
    "[rev e2e 단계 2] PR {pr_numbers} dev 배포 E2E 검증 launch — "
    "develop dev 배포 후 배포본 대상 E2E (spec docs/features/stage2-dev-deploy-e2e.md §5-4). "
    "통과 시 라벨 `rev-post-merge-pass` 부여."
)
# Discord push template (#1447 — 로그형 → narrative, '감사' 금지).
REV_POST_MERGE_AUDIT_DISCORD_TEMPLATE: Final[str] = (
    "🔍 PR {pr_numbers} 이(가) develop 에 머지됐으므로, dev 배포본에 통합·배포 회귀가 없는지 "
    "dev 배포 E2E 검증(rev 단계 2)을 진행합니다. rev 큐에 적재했고, 결과는 rev forum 에서 "
    "확인하실 수 있습니다."
)
...
def load_env() -> dict[str, str]:
    ...
    # rev e2e 단계 2 (dev 배포 E2E 검증) loop (#1008)
...
def fetch_rev_post_merge_candidates(
    *,
    ...
    """develop 머지된 PR 중 단계 2 (dev 배포 E2E 검증) 필요한 PR 번호 리스트를
    반환합니다.
    ...
    Args:
        pass_label: 단계 2 (dev 배포 E2E 검증) 통과 라벨 (rev sub-agent 가 부여).
            이 라벨이 부재한 PR 만 후보로 잡힙니다.
        search_window: gh CLI `merged:>${X} ago` 윈도우 — 너무 오래된 머지는
            polling 부담만 누적되므로 1시간만 본다 (단계 2 (dev 배포 E2E 검증)
...
def build_client(env: dict[str, str], ledger: DedupLedger | None) -> discord.Client:
    ...
    async def on_ready() -> None:  # noqa: D401
        ...
        # rev e2e 단계 2 (dev 배포 E2E 검증) 자동 trigger (#1008).
        # 5분 polling — develop 머지된 PR 단계 2 (dev 배포 E2E 검증) 자동 launch.
        # spec: docs/features/rev-e2e-2-stages.md §3-2.
