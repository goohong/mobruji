...
THREAD_CLEANUP_DEFAULT_INTERVAL_SECONDS: Final[int] = 300  # 5분
    ...
    while True:
        try:
            if cached_guild_id is None:
                ...
                if cached_guild_id is None:
                    logger.debug(
                        "thread_cleanup_loop: guild_id 미상 — 다음 iter 재시도",
                    )
            ...
            logger.info(
                "thread_cleanup_loop: scanned=%d archived=%d deleted=%d skipped=%d (keep=%d age=%s)",
                scanned,
                archived,
                deleted,
                skipped,
            ...
            logger.warning("thread_cleanup_loop iter 실패: %s", exc)
        await sleeper(poll_interval)
...
def build_client(env: dict[str, str], ledger: DedupLedger | None) -> discord.Client:
    ...
    async def on_ready() -> None:  # noqa: D401
        ...
        # default age=15min interval=5min keep=3 archive (delete 옵션 별도 toggle).
