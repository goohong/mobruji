...
async def ensure_mode_toggle_message(
    ...
    bot boot 시 1회 호출. 기존 message 가 있으면 ``client.add_view(view, message_id=...)`` 로
    persistent 재bind. 없으면 신규 post + 동일 view attach.
...
def build_client(env: dict[str, str], ledger: DedupLedger | None) -> discord.Client:
    ...
    async def on_ready() -> None:  # noqa: D401
        ...
        if not hasattr(client, "_mode_toggle_view_attached"):
            ...
            logger.info(
                "mode toggle persistent view attached (custom_ids: mobruji-mode-ask, mobruji-mode-auto)"
            )
