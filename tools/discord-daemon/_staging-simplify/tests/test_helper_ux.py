...
class DiscordReplyResolvePriorityTests(unittest.TestCase):
    """`resolve_reply_to_id` 우선순위 체인 검증 (단순화본 — refactor/bot-py-simplify-remove-wrappers).

    5+분 turn 에서 freeze / queue pending 은 stale 만 되어 useful 하지 않다는
    사용자 판단에 따라 5단 → 2단으로 단순화 (helper-current-target.txt freeze,
    HELPER_TURN_TARGET_MSG_ID env, helper-queue.jsonl pending 폐기).

    우선순위 (높음 → 낮음):
      1. --reply-to <id>
      2. last-user-msg-id.txt
    """
    ...
    def test_env_var_no_longer_overrides_last_id(self) -> None:
        """HELPER_TURN_TARGET_MSG_ID env 는 더 이상 인식되지 않는다 (단순화 후).

        last-id 만 fallback 으로 사용. helper turn 5+분 진행 중에 env 가 stale 만
        됐기 때문 (사용자 결정).
        """
        result, capture_path = self._run(
            "body",
        ...
        # env 무시 → last-id 사용.
        self.assertEqual(
            payload["message_reference"]["message_id"], "22222222222222222"
        )

    def test_target_file_freeze_no_longer_used(self) -> None:
        """helper-current-target.txt freeze 는 단순화로 제거됨.

        설령 파일이 존재해도 무시. last-id 만 사용.
        """
        result, capture_path = self._run(
            "body",
            last_id_content="12345678901234567",
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        payload = self._first_payload(capture_path)
        # freeze 무시 → last-id 사용.
        self.assertEqual(
            payload["message_reference"]["message_id"], "12345678901234567"
        )

    def test_queue_pending_no_longer_used(self) -> None:
        """helper-queue.jsonl pending entry 는 단순화로 제거됨.

        queue 에 entry 가 있어도 무시. last-id 만 사용.
        """
        result, capture_path = self._run(
            "body",
