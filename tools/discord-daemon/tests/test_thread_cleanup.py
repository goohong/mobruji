...
# ─────────────────────────────────────────────────────────────────────────────
# 6) #1062 silent root cause logging 검증 — 2건
# ─────────────────────────────────────────────────────────────────────────────
#
# silent root cause fix: guild_id 미상 시 debug → warning 격상 + 매 iter
# 진입 시 INFO 로그. 사용자 "주기 청소 안되고 있네" 보고 시 journal grep
# 으로 즉시 진단 가능.


class ThreadCleanupSilentLoggingTest(unittest.IsolatedAsyncioTestCase):

    async def test_guild_id_none_emits_warning(self) -> None:
        """#1062 silent fix: guild_id None 일 때 warning 로그 emit."""
        async def noop_sleep(_):
            await asyncio.sleep(0)
        ...
        coro = bot.thread_cleanup_loop(
            ...
            poll_interval=1,
            initial_delay=0,
            guild_id_fetcher=lambda: None,  # 항상 None
            threads_fetcher=lambda gid: [],
            archiver=lambda tid: True,
            sleeper=noop_sleep,
        )
        with self.assertLogs("mobruji-discord-daemon", level="WARNING") as cm:
            await _run_iters(coro, iterations=5)
        # warning 메시지 중 'guild_id 미상' 어구 포함.
        warnings_text = "\n".join(cm.output)
        self.assertIn("guild_id 미상", warnings_text)

    async def test_iter_entry_emits_info_log(self) -> None:
        """#1062 silent fix: iter 진입마다 INFO 로그 emit — alive 신호."""
        async def noop_sleep(_):
            await asyncio.sleep(0)
        ...
        with self.assertLogs("mobruji-discord-daemon", level="INFO") as cm:
            await _run_iters(coro, iterations=5)
        info_text = "\n".join(cm.output)
        # launched + iter=N scan 시작 둘 다 가시.
        self.assertIn("launched", info_text)
        self.assertIn("scan 시작", info_text)


# ─────────────────────────────────────────────────────────────────────────────
# 7) #1062 interval default 2분 검증 — 1건
# ─────────────────────────────────────────────────────────────────────────────


class ThreadCleanupIntervalDefaultTest(unittest.TestCase):

    def test_default_interval_is_120s(self) -> None:
        """#1062 사용자 P1: default interval 5min → 2min (120s)."""
        self.assertEqual(bot.THREAD_CLEANUP_DEFAULT_INTERVAL_SECONDS, 120)
